...
function StoreGetInfo(){
	...
	request_get.done(function(data){
		if(data.OK == 0){
			...
			if(data.OK == 1){
			$('.change-name-title span:nth-child(2)').append(data.info.name);
			$('.change-phone-title span:nth-child(2)').append(data.info.mobile);
			$('.change-email-title span:nth-child(2)').append(data.info.email);
			$('.change-shop-title span:nth-child(2)').append(data.info.city + '&nbsp;&nbsp;' + data.info.store+'</br>');
		}
	});
}
