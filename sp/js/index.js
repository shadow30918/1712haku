...
stopDrop();
	/*$(window).scroll(function(){
		if($(window).scrollTop()<1){
			$('.visual-img').css('position','fixed');
			//$("body").on("touchmove",function(event){
			//	event.preventDefault;
			//}, false)
		}else {
			$('.visual-img').css('position','absolute');
			//$("body").off("touchmove");
		}
	});*/
  ...
  //index
	...
	$('.menu , .close, .link-yt, .link-blog, .link-prod, .link-game').click(function(e){
	...
	$('.game-img').click(function(){
		...
		$('body').css({'position':'fixed','top': -winscroll });
		$('.info').css('display','block');
		
	});

	$('.info-close,.close-bg').click(function(){
  ...
  /*
	...
	*/
	...
	/*$('.visual-img').delay(100).fadeIn(500);*/
	$('.visual-img img').css('opacity','1');
	$('.visual-img img').css('width','105%');
	$('.logo').delay(800).fadeIn(800);
	$('.title').delay(1200).fadeIn(800);
  ...
  $('.link-game').click(function(e){
		e.preventDefault();
		$('html,body').animate({
			scrollTop:$('.game').offset().top - 56
		},800);
		/*$('.menu-bg').slideUp(800);
		$('.menu').css('display','inline-block');
		$('.close').css('display','none');*/
	...
	$('.back-btn').click(function(){
		$('.show-info').remove();
		$('.done-title,.done-info,.done-check,.done-btn,.done-notes').fadeOut(0);
  ...
  $(window).resize(function(){
  	if( w != $(window).width() ){
		$('.menu-bg').removeClass("opened");
  	  	...
  	  	if(w < 767){
			var sScale = (w/1090) * 0.8;
			var canvasScale = 'translate(-50%,-50%)' + 'scale('+ sScale + ')';

			$('.canvasContainer').css('transform',canvasScale);
		}else {
			var sScale = (w/1090) * 0.7;
			var canvasScale = 'translate(-50%,-50%)' + 'scale('+ sScale + ')';
			$('.canvasContainer').css('transform',canvasScale);
		}
  ...
  function CityStoreListByEvent(){
	...
	function checkData(){
      ...
      if(c_no == '' || s_no == ''){
		...
		if(check_format == 1){
			$('.info-words,.input,.agree,.info-btn').fadeOut(0);
			$('.done-title,.done-info,.done-check,.done-btn').fadeIn(300);
			$('.done-check .name').append('<span class="show-info">' + $('#name').val() + '</span>');
			$('.done-check .phone').append('<span class="show-info">' + $('#phone').val() + '</span>');
			$('.done-check .email').append('<span class="show-info">' + $('#email').val() + '</span>');
         $('.done-check .shop').append('<span class="show-info">' + $('#city option:selected').text() + ' ' + $('#shop option:selected').attr('text') + '</span>');
	...
	function SubmitForm(){
		...
		request_sub.done(function(data){
			...
			if(re_ok == 1){
				...
				if(typeof(window.adGeek) != 'undefined' && typeof(window.adGeek.conversion) == 'function'){
				    window.adGeek.conversion({callback: function () {
						window.location = "https://www.shiseido-event.com/2017haku/thank.html?c_no=" + c_no + "&s_no=" + s_no + "&t_no=" + t_no + "&t_sn_no=" + t_sn_no + '&token=' + token; 
				}});
				}else{
				   window.location = "https://www.shiseido-event.com/2017haku/thank.html?c_no=" + c_no + "&s_no=" + s_no + "&t_no=" + t_no + "&t_sn_no=" + t_sn_no + '&token=' + token; 
				}
